...
<div class="tab-content pt-3" id="tabContent">
    ...
    <div class="tab-pane fade" id="inspeccion" role="tabpanel">
        ...
        <table class="table table-bordered">
            <thead><tr><th>Táctico</th><th>Gestor</th><th>PDV</th><th>Motivo</th><th>Fecha</th><th>Fecha Reportado</th></tr></thead>
            <tbody id="reportesTableBody"></tbody>
        </table>

        <h5 class="mt-4">Registros Validados</h5>
        <table class="table table-bordered">
            <thead><tr><th>Táctico</th><th>Gestor</th><th>PDV</th><th>Cantidad Armada</th><th>Fecha</th><th>Fecha Validado</th></tr></thead>
...
<script>
    ...
    for (const tipo in data) {
        ...
        if (tipo === 'ejecutivo') {
            // Cabecera personalizada para EJECUTIVO
            html += `
                <th>JEFATURA</th>
                <th>EJECUTIVO</th>
        ...
        // ---------------------------
        // 1️⃣ EJECUTIVO CON SUBTOTALES Y FALTANTES
        // ---------------------------
        if (tipo === 'ejecutivo') {
            const campoAgrupado = 'jefatura';
            let actual = null;
            let subtotal = { recursos: 0, distrib: 0, armada: 0, faltante: 0 };

            grupo.forEach((row, index) => {
                const recursos = parseFloat(row.cantidad_recursos || 0);
                const distrib = parseFloat(row.cantidad_distribuida || 0);
                const armada = parseFloat(row.cantidad_armada || 0);
                const faltante = distrib - armada;

                subtotal.recursos += recursos;
                subtotal.distrib += distrib;
                subtotal.armada += armada;
                subtotal.faltante += faltante;

                html += '<tr>';
                if (row[campoAgrupado] !== actual) {
                    const rowspan = grupo.filter(r => r[campoAgrupado] === row[campoAgrupado]).length;
                    html += `<td rowspan="${rowspan + 1}">${row[campoAgrupado]}</td>`; // +1 para subtotal
                    actual = row[campoAgrupado];
                }

                // Columnas en el nuevo orden
                html += `
                    ...
                    <td>${armada}</td>
                    <td>${faltante}</td>
                    <td>${getProgressBar(row.avance)}</td>
                `;
                html += '</tr>';

                // Cuando cambia de grupo o es el último → imprimir subtotal
                const siguiente = grupo[index + 1];
                if (!siguiente || siguiente[campoAgrupado] !== actual) {
                    const avanceSubtotal = subtotal.distrib === 0
                        ? 0
                        : ((subtotal.armada / subtotal.distrib) * 100).toFixed(2);

                    html += `<tr class="table-secondary fw-bold">
                        <td colspan="1">Subtotal ${actual}</td>
                        <td>${subtotal.recursos}</td>
                        <td>${subtotal.distrib}</td>
                        <td>${subtotal.armada}</td>
                        <td>${subtotal.faltante}</td>
                        <td>${avanceSubtotal}%</td>
                    </tr>`;

                    subtotal = { recursos: 0, distrib: 0, armada: 0, faltante: 0 };
                }
            });

        } 
        // ---------------------------
        // 2️⃣ MERCADERISTA / REGIONAL → ORIGINAL
        // ---------------------------
        else {
            const campoAgrupado = tipo === 'mercaderista' ? 'supervisor' : null;
            let actual = null;
            const headers = Object.keys(grupo[0]);

            grupo.forEach(row => {
                html += '<tr>';
                if (campoAgrupado && row[campoAgrupado] !== actual) {
                    const rowspan = grupo.filter(r => r[campoAgrupado] === row[campoAgrupado]).length;
                    html += `<td rowspan="${rowspan}">${row[campoAgrupado]}</td>`;
                    actual = row[campoAgrupado];
                }
                headers.slice(campoAgrupado ? 1 : 0).forEach(h => {
                    html += h === 'avance'
    ...
    const tipo = document.getElementById('tipoTactico').value;
    fetch(`../get_gestores_nuevo.php?es_adicional=${tipo}`)  // ✅ Agregado paréntesis de apertura
        .then(response => {
            console.log("Status:", response.status);
            return response.text(); // Temporalmente cambiado a text() para debug
        })
        .then(text => {
   ...
   function toggleEstadoTactico(id_packs, activar) {
    ...
    function cargarCorregidos() {
        ...
        fetch(`../get_registros_corregidos_new.php?es_adicional=${tipo}`)
            .then(response => response.json())
            .then(response => {
