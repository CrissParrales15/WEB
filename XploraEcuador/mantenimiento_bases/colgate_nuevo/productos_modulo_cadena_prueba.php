...
if (isset($_POST["import"])) {
    ...
    if ($_FILES["file"]["size"] > 0) {
		...
		$sqlTruncate = "TRUNCATE table repositorio_productos_modulo_cadena_prueba_normalizacion;";
		...
		while (($column = fgetcsv($file, 10000, ";")) !== FALSE) {
			...
			$modulo = "";
			if (isset($column[2])) {
				$modulo = mysqli_real_escape_string($conn, $column[2]);
			}
			...
			$activo = "";
			if (isset($column[4])) {
				$activo = mysqli_real_escape_string($conn, $column[4]);
			}

			$eliminar = "";
			if (isset($column[5])) {
				$eliminar = mysqli_real_escape_string($conn, $column[5]);
			}
...
function insertBatch($conn, $batchData) {
	...
	$sql = "INSERT INTO repositorio_productos_modulo_cadena_prueba_normalizacion 
			(id, codigosku, modulo, re, activo, eliminar) VALUES ";
...
<body>
	...
	<div id="content" style="width:100%;">
		<nav class="navbar navbar-expand-lg navbar-light bg-light">
			<div class="container-fluid">
				<h3>Base Productos Modulo Cadenas (PRUEBA RE SEPARADO POR COMAS) - INSERCIÓN POR LOTES</h3>
			</div>
		</nav>
	...
	<script>
		$(document).ready(function() {
			$.ajax({
				type: "POST",
				url: "getters/get_table_productos_modulo_cadena_prueba.php",
				data: null,
				beforeSend: function() {
