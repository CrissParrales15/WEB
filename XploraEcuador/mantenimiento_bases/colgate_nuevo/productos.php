...
if (isset($_POST["import"])) {
    ...
    if ($_FILES["file"]["size"] > 0) {
		...
		$sqlTruncate = "TRUNCATE table repositorio_productos;";
		...
		while (($column = fgetcsv($file, 10000, ";")) !== FALSE) {
            ...
            $codigosku = "";
			if (isset($column[1])) {
				$codigosku = mysqli_real_escape_string($conn, $column[1]);
			}
			...
			$forma = "";
			if (isset($column[5])) {
				$forma = mysqli_real_escape_string($conn, $column[5]);
			}

			$fabricante = "";
			if (isset($column[6])) {
				$fabricante = mysqli_real_escape_string($conn, $column[6]);
			}

			$marca = "";
			if (isset($column[7])) {
				$marca = mysqli_real_escape_string($conn, $column[7]);
			}

			$variante = "";
			if (isset($column[8])) {
				$variante = mysqli_real_escape_string($conn, $column[8]);
			}

			$ean = "";
			if (isset($column[9])) {
				$ean = mysqli_real_escape_string($conn, $column[9]);
			}

			$cod_sap = "";
			if (isset($column[10])) {
				$cod_sap = mysqli_real_escape_string($conn, $column[10]);
			}

			$sku = "";
			if (isset($column[11])) {
				$sku = mysqli_real_escape_string($conn, $column[11]);
			}
			
			$sku_sugerido = "";
			if (isset($column[12])) {
				$sku_sugerido = mysqli_real_escape_string($conn, $column[12]);
			}
			
			$consumer_unit = "";
			if (isset($column[13])) {
				$consumer_unit = mysqli_real_escape_string($conn, $column[13]);
			}
			
			$tamano = "";
			if (isset($column[14])) {
				$tamano = mysqli_real_escape_string($conn, $column[14]);
			}

			$nuevo = "";
			if (isset($column[15])) {
				$nuevo = mysqli_real_escape_string($conn, $column[15]);
			}

			$backup = "";
			if (isset($column[16])) {
				$backup = mysqli_real_escape_string($conn, $column[16]);
			}

			$activar = "";
			if (isset($column[17])) {
				$activar = mysqli_real_escape_string($conn, $column[17]);
			}
			
            $sqlInsert = "INSERT INTO repositorio_productos (id,codigosku,categoria,subcategoria,segmento,forma,fabricante,marca,variante,ean,cod_sap,sku,sku_sugerido,consumer_unit,tamano,nuevo,backup,activar) VALUES  (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
            $paramType = "isssssssssssssssss";
            $paramArray = array(
                $id,
				$codigosku,
				$categoria,
...
<body>
	...
	<script>
		$(document).ready(function() {
			$.ajax({
				type: "POST",
				url: "getters/get_table_productos.php",
				data: null,
				beforeSend: function() {
					$("#loading").css("display", "block");
				},
