...
if (isset($_POST["import"])) {
    ...
    if ($_FILES["file"]["size"] > 0) {
		...
		while (($column = fgetcsv($file, 10000, ";")) !== FALSE) {
			...
			$tipo_actividad = "";
			if (isset($column[2])) {
				$tipo_actividad = mysqli_real_escape_string($conn, $column[2]);
			}
            ...
            $sqlInsert = "INSERT INTO repositorio_tipo_actividad (id,re,tipo_actividad,activo,eliminar) VALUES  (?,?,?,?,?)";
            $paramType = "issss";
            $paramArray = array(
                $id,
				$re,
...
<body>
	...
	<script>
		$(document).ready(function() {
			$.ajax({
				type: "POST",
				url: "getters/get_table_tipo_actividad.php",
				data: null,
				beforeSend: function() {
