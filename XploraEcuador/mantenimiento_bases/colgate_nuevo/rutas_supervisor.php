...
<body>
    ...
    <form method="post" action="" name="frmCSVImport" id="frmCSVImport" enctype="multipart/form-data">
        <div class="wrapper">
                ...
                <ul class="list-unstyled components">
                    ...
                    <li class="active">
                        <div class="form-group" style="margin-left:5%;margin-right:5%;">
                            ...
                            <select class="form-control js-example-basic-single" id="codigos_pdv" name="codigos_pdv" required></select>
                        </div>
                    </li>
                    <li class="active">
                        <div class="form-group" style="margin-left:5%;margin-right:5%;">
                    ...
                    <li class="active" style="color:black;">
                        <div class="form-group" style="margin-left:5%;margin-right:5%;">
                            <label for="apoyo" style="color:white;">Punto de Apoyo</label>
                            <ul class="list-group apoyo">
                                <li class="list-group-item">
                                    <input class="form-check-input me-1" type="checkbox" value="1" aria-label="..." id="chkApoyo" name="apoyo">
                                    Punto de Apoyo
                                </li>
                            </ul>
                        </div>
            ...
            <div id="content" style="width:100%;">
                ...
                <h3>Consultar rutas Supervisor</h3>
                ...
                <div class="row" style="text-align: center; margin-bottom:1%;">
                    <div class="col-xl-6 col-lg-6 mb-6">
                        <h2>Rutero Supervisor</h2>
                    </div>
                    <div class="col-xl-6 col-lg-6 mb-6">
                        <a class="btn btn-primary" href="https://webecuador.azurewebsites.net/App/XploraEcuador/mantenimiento_bases/colgate_nuevo/formatosSupervisor.rar" role="button">Descargar formato</a>
            ...
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    ...
                    <div class="modal-body mx-3">
                        ...
                        <div class="md-form mb-2">
                            <label data-error="wrong" data-success="right" for="fechaEdit">Codigo PDV</label>
                            <select class="form-control js-example-basic-single" id="codigoEdit" name="codigos_pdv"></select>
                        </div>
                        ...
                        <div class="md-form mb-2">
                            <label data-error="wrong" data-success="right" for="usuarioEdit">Estado</label>
    ...
    <script>
        function deleteRow(id) {
            $.ajax({
                type: "POST",
                url: "updates/delete_ruta_supervisores.php",
                data: {
                    id: id
                },
                beforeSend: function() {
        ...
        function getUsuarios() {
            // formdata = "&codigo_pdv=" + codigo_pdv;

            $.ajax({
                type: "POST",
                url: "getters/getUsuariosSupervisores.php",
                data: null,
                beforeSend: function() {
                    $("#loading").css("display", "block");
                },
        ...
        function buscarRuta() {
            ...
            if ($("#fechaInicioFiltro").val() && $("#fechaFinFiltro").val()) {
                ...
                $.ajax({
                    type: "POST",
                    url: "get_table_rutero_supervisores.php",
                    data: {
        ...
        function saveRutero(fecha_inicio, fecha_fin, codigo_pdv, usuario, id_supervisor, days, apoyo) {
            formdata = "&fecha_inicio=" + fecha_inicio + "&fecha_fin=" + fecha_fin + "&codigo_pdv=" + codigo_pdv + "&usuario=" + usuario + "&id_supervisor=" + id_supervisor + "&days=" + days + "&apoyo=" + apoyo;

            $.ajax({
                type: "POST",
                url: "rutero_pdvs_supervisores.php",
                data: formdata,
                beforeSend: function() {
                    $("#loading").css("display", "block");
                },
        ...
        function editRuta(id, fecha, codigo, usuario, habilitado) {
            formdata = "&id=" + id + "&fecha=" + fecha + "&codigo=" + codigo + "&usuario=" + usuario + "&habilitado=" + habilitado;

            $.ajax({
                type: "POST",
                url: "updates/update_ruta_supervisores.php",
                data: formdata,
                beforeSend: function() {
                    $("#loading").css("display", "block");
                },
        ...
        function resetFormulario() {
            ...
            $("#chkApoyo").prop("checked", false);
        }

        function esFormularioValido(fecha_inicio, fecha_fin, codigo_pdv, usuario, id_supervisor, days) {
            if (!fecha_inicio) {
                Swal.fire({
                    // icon: 'info',
                    title: 'Nueva ruta',
        ...
        $(document).ready(function() {
            ...
            $("#save").click(function() {
                ...
                var codigo_pdv = $('#codigos_pdv').val();
                var usuario = $('#usuarios').val();
                var id_supervisor = $('#usuarios').val();
                ...
                var apoyo = $('#chkApoyo').is(":checked") ? '1' : '0';
                $('#apoyo').val(apoyo); // Establecer el valor del campo oculto para enviar al servidor

                if (esFormularioValido(fecha_inicio, fecha_fin, codigo_pdv, usuario, id_supervisor, days)) {
                    saveRutero(fecha_inicio, fecha_fin, codigo_pdv, usuario, id_supervisor, days, apoyo);
                }
            });
            ...
            $("button").click(function(event) {
                if ($(this).attr("value") == "search") {
                    ...
                    editRuta(id, fecha, codigo, usuario, habilitado);
                }
            ...
            $("#frmCSVImport button[type = 'submit']").click(function(e) {
                ...
                if (buttonClicked == "import_nueva_ruta") {
                    ...
                    buttonClicked = "";
                    $.ajax({
                        url: "cargar_modificar_ruta_supervisores.php",
                        method: "POST",
                        data: form_data,
                        contentType: false, // The content type used when sending data to the server.  
                        cache: false, // To unable request pages to be cached  
