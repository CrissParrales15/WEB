...
if (isset($_POST["import"])) {
    ...
    if ($_FILES["file"]["size"] > 0) {
		...
		$sqlTruncate = "TRUNCATE table repositorio_rutas;";
		...
		while (($column = fgetcsv($file, 10000, ";")) !== FALSE) {
            ...
            $rutas = "";
			if (isset($column[1])) {
				$rutas = mysqli_real_escape_string($conn, $column[1]);
			}
            ...
            $sqlInsert = "INSERT INTO repositorio_rutas (id,rutas,activo,eliminar) VALUES  (?,?,?,?)";
            $paramType = "isss";
            $paramArray = array(
                $id,
				$rutas,
...
<body>
	...
	<script>
		$(document).ready(function() {
			$.ajax({
				type: "POST",
				url: "getters/get_table_repositorio_rutas.php",
				data: null,
				beforeSend: function() {
