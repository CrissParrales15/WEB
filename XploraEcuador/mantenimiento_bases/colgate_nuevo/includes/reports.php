...
function malla_reporte($fechaInicio, $fechaFin, $mysqli)
{
	$fecha = '%' . $fecha;
	if ($sql = $mysqli->prepare(
		"SELECT 
		...
		ins.codifica,
		ins.fechaservidor 
		FROM insert_malla_codificados ins 
		...
		if ($sql->num_rows > 0) {
           ...
           ';
			...
			$output .= '</table>';
			header("Content-Type: application/xls");
			header("Content-Disposition: attachment; filename=ReporteMallaCodificados.xls");
			echo $output;
		} else {
			echo "<script>
	alert('No se encontraron registros.');
...
")) {
		...
		if ($sql->num_rows > 0) {
           ...
           ';
			$img_url = "https://luckyecuadorweb.blob.core.windows.net/app/AppDanec/Inserts/";
			while ($sql->fetch()) {
				...
				$output .= '  
                     <tr>  		
						...
						<td style="height:20px">' . utf8_decode($tipo) . '</td> 
						<td style="height:20px">' . utf8_decode($causal) . '</td> 
						<td style="height:20px">' . $img_url.$foto_registro . '</td> 
			...
			while ($sql->fetch()) {
				...
				$output .= '  
                     <tr>  
						...
						<td style="height:20px">' . utf8_decode($razon) . '</td> 
						<td style="height:20px">' . utf8_decode($fechaservidor) . '</td> 
                     </tr>  
                ';
			}
