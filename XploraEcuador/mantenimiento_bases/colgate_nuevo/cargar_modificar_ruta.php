	...
	if (!empty($_FILES['file_modificar_ruta']['name']) && in_array($_FILES['file_modificar_ruta']['type'], $fileMimes)) {
		...
		if ($_FILES["file_modificar_ruta"]["size"] > 0) {
			...
			while (($column = fgetcsv($file, 27000, ";")) !== FALSE) {
				...
				$sqlInsert = "UPDATE rutero_pdv SET id_usuario=?, id_pdv=?, id_supervisor=(SELECT supervisor FROM repositorio_locales_dtt2 WHERE id=?), fecha_visita=?, status=?, habilitado=?, usuario_edicion=? WHERE id=?";
				$paramType = "iiisiisi";
				$paramArray = array(
					$id_usuario, 
					$id_pdv, 
