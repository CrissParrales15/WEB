...
if (isset($_POST["import"])) {
    ...
    if ($_FILES["file"]["size"] > 0) {
		...
		$sqlTruncate = "TRUNCATE table repositorio_material_pop;";
		...
		while (($column = fgetcsv($file, 10000, ";")) !== FALSE) {
			...
			$materialpop = "";
			if (isset($column[2])) {
				$materialpop = mysqli_real_escape_string($conn, $column[2]);
			}

			$fabricante = "";
			if (isset($column[3])) {
				$fabricante = mysqli_real_escape_string($conn, $column[3]);
			}
			...
			$activo = "";
			if (isset($column[6])) {
				$activo = mysqli_real_escape_string($conn, $column[6]);
			}

			$eliminado = "";
			if (isset($column[7])) {
				$eliminado = mysqli_real_escape_string($conn, $column[7]);
			}
			
			
            $sqlInsert = "INSERT INTO repositorio_material_pop (id,re,materialpop,fabricante,categoria,subcategoria,activo,eliminado) VALUES  (?,?,?,?,?,?,?,?)";
            $paramType = "isssssss";
            $paramArray = array(
                $id,
				$re,
...
<body>
	...
	<script>
		$(document).ready(function() {
			$.ajax({
				type: "POST",
				url: "getters/get_table_materialpop.php",
				data: null,
				beforeSend: function() {
