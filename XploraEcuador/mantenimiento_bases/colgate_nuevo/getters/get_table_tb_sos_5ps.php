<?php
// getters/get_table_tb_sos_5ps.php (SERVER-SIDE PROCESSING)
...
if ($stmt = $mysqli->prepare($sql_data)) {
    ...
    while ($row = $result->fetch_assoc()) {
        $row_data = [
            ...
            $row['subcategory'],
            $row['total_cms_hooks'],
            $row['cms_individual_hooks'],
            $row['sos'],
            $row['server_date'],
            $row['fecha_modificacion'],
            '<button class="btn btn-sm btn-primary btn-editar-sos" data-id="' . htmlspecialchars($row['id'], ENT_QUOTES) . '">
