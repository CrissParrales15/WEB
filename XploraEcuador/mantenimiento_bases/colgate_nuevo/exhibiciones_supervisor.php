...
if (isset($_POST["import"])) {
    ...
    if ($_FILES["file"]["size"] > 0) {
		...
		$sqlTruncate = "TRUNCATE table repositorio_exhibicion_supervisor2;";
		...
		while (($column = fgetcsv($file, 10000, ";")) !== FALSE) {
            ...
            $cp_code = "";
			if (isset($column[1])) {
				$cp_code = mysqli_real_escape_string($conn, $column[1]);
			}

            $local = "";
			if (isset($column[2])) {
				$local = mysqli_real_escape_string($conn, $column[2]);
			}

            $visual = "";
			if (isset($column[3])) {
				$visual = mysqli_real_escape_string($conn, $column[3]);
			}

			$canal = "";
			if (isset($column[4])) {
				$canal = mysqli_real_escape_string($conn, $column[4]);
			}

			$supervisor = "";
			if (isset($column[5])) {
				$supervisor = mysqli_real_escape_string($conn, $column[5]);
			}

			$exhibition_tool = "";
			if (isset($column[6])) {
				$exhibition_tool = mysqli_real_escape_string($conn, $column[6]);
			}

			$manufacturer = "";
			if (isset($column[7])) {
				$manufacturer = mysqli_real_escape_string($conn, $column[7]);
			}

			$category = "";
			if (isset($column[8])) {
				$category = mysqli_real_escape_string($conn, $column[8]);
			}

			$subcategory = "";
			if (isset($column[9])) {
				$subcategory = mysqli_real_escape_string($conn, $column[9]);
			}

			$zone_exhibition = "";
			if (isset($column[10])) {
				$zone_exhibition = mysqli_real_escape_string($conn, $column[10]);
			}

			$personalization = "";
			if (isset($column[11])) {
				$personalization = mysqli_real_escape_string($conn, $column[11]);
			}

			$price_tag = "";
			if (isset($column[12])) {
				$price_tag = mysqli_real_escape_string($conn, $column[12]);
			}
			
			$num_exhibitions = "";
			if (isset($column[13])) {
				$num_exhibitions = mysqli_real_escape_string($conn, $column[13]);
			}
			
			$observation = "";
			if (isset($column[14])) {
				$observation = mysqli_real_escape_string($conn, $column[14]);
			}
			...
			$date = "";
			if (isset($column[16])) {
				$date = mysqli_real_escape_string($conn, $column[16]);
			}
			
			$visual_access = "";
			if (isset($column[17])) {
				$visual_access = mysqli_real_escape_string($conn, $column[17]);
			}
			
			$photo2 = "";
			if (isset($column[18])) {
				$photo2 = mysqli_real_escape_string($conn, $column[18]);
			}
			
			$tipo_herramienta = "";
			if (isset($column[19])) {
				$tipo_herramienta = mysqli_real_escape_string($conn, $column[19]);
			}
			
			$convenio = "";
			if (isset($column[20])) {
				$convenio = mysqli_real_escape_string($conn, $column[20]);
			}
			
			$clasificacion = "";
			if (isset($column[21])) {
				$clasificacion = mysqli_real_escape_string($conn, $column[21]);
			}
			
			$campana = "";
			if (isset($column[22])) {
				$campana = mysqli_real_escape_string($conn, $column[22]);
			}
			
			$comentario = "";
			if (isset($column[23])) {
				$comentario = mysqli_real_escape_string($conn, $column[23]);
			}
			
			$activo = "";
			if (isset($column[24])) {
				$activo = mysqli_real_escape_string($conn, $column[24]);
			}
			
			$eliminado = "";
			if (isset($column[25])) {
				$eliminado = mysqli_real_escape_string($conn, $column[25]);
			}
			
			
            $sqlInsert = "INSERT INTO repositorio_exhibicion_supervisor2 (id,cp_code,local,visual,canal,supervisor,exhibition_tool,manufacturer,category,subcategory,zone_exhibition,personalization,price_tag,num_exhibitions,observation,photo,date,visual_access,photo2,tipo_herramienta,convenio,clasificacion,campana,comentario,activo,eliminado) VALUES  (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
            $paramType = "isssssssssssssssssssssssss";
            $paramArray = array(
                $id,
				$cp_code,
				$local,
...
<body>
	...
	<div id="content" style="width:100%;">
		<nav class="navbar navbar-expand-lg navbar-light bg-light">
			<div class="container-fluid">
				<h3>Base Exhibiciones Pre-Cargada</h3>
			</div>
		</nav>
	...
	<script>
		$(document).ready(function() {
			$.ajax({
				type: "POST",
				url: "getters/get_table_exhibiciones_supervisor.php",
				data: null,
				beforeSend: function() {
