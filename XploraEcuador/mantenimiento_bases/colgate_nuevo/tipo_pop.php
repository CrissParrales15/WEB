...
if (isset($_POST["import"])) {
    ...
    if ($_FILES["file"]["size"] > 0) {
		...
		$sqlTruncate = "TRUNCATE table repositorio_tipopop;";
		...
		while (($column = fgetcsv($file, 10000, ";")) !== FALSE) {
            ...
            $apoyopop = "";
			if (isset($column[1])) {
				$apoyopop = mysqli_real_escape_string($conn, $column[1]);
			}
            ...
            $sqlInsert = "INSERT INTO repositorio_tipopop (id,apoyopop,activo,eliminar) VALUES  (?,?,?,?)";
            $paramType = "isss";
            $paramArray = array(
                $id,
				$apoyopop,
...
<body>
	...
	<script>
		$(document).ready(function() {
			$.ajax({
				type: "POST",
				url: "getters/get_table_tipopop.php",
				data: null,
				beforeSend: function() {
