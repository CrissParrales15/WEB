...
if (isset($_POST["import"])) {
    ...
    if ($_FILES["file"]["size"] > 0) {
		...
		$sqlTruncate = "TRUNCATE table repositorio_evaluacion_encuestas;";
		...
		while (($column = fgetcsv($file, 10000, ";")) !== FALSE) {
            ...
            $nombre_encuesta = "";
			if (isset($column[0])) {
				$nombre_encuesta = mysqli_real_escape_string($conn, $column[0]);
			}

			$descripcion= "";
			if (isset($column[1])) {
				$descripcion = mysqli_real_escape_string($conn, $column[1]);
			}
            ...
            $re = "";
			if (isset($column[3])) {
				$re = mysqli_real_escape_string($conn, $column[3]);
			}

			$pregunta = "";
			if (isset($column[4])) {
				$pregunta = mysqli_real_escape_string($conn, $column[4]);
			}

			$tipo_pregunta = "";
            if (isset($column[5])) {
                $tipo_pregunta = mysqli_real_escape_string($conn, $column[5]);
            }

			$opc_a = "";
            if (isset($column[6])) {
                $opc_a = mysqli_real_escape_string($conn, $column[6]);
            }

			$opc_b = "";
            if (isset($column[7])) {
                $opc_b = mysqli_real_escape_string($conn, $column[7]);
            }

			$opc_c = "";
            if (isset($column[8])) {
                $opc_c = mysqli_real_escape_string($conn, $column[8]);
            }

			$opc_d = "";
            if (isset($column[9])) {
                $opc_d = mysqli_real_escape_string($conn, $column[9]);
            }

			$opc_e = "";
            if (isset($column[10])) {
                $opc_e = mysqli_real_escape_string($conn, $column[10]);
            }

			$foto = "";
            if (isset($column[11])) {
                $foto = mysqli_real_escape_string($conn, $column[11]);
            }

			$tipo_campo = "";
            if (isset($column[12])) {
                $tipo_campo = mysqli_real_escape_string($conn, $column[12]);
            }


/*             $puntaje_por_pregunta = "";
			if (isset($column[6])) {
				$puntaje_por_pregunta = mysqli_real_escape_string($conn, $column[6]);
			}

			$cuenta = "";
			if (isset($column[7])) {
				$cuenta = mysqli_real_escape_string($conn, $column[7]);
			} */

			$habilitado = "";
			if (isset($column[13])) {
				$habilitado = mysqli_real_escape_string($conn, $column[13]);
			}
			

            $sqlInsert = "INSERT INTO repositorio_evaluacion_encuestas (nombre_encuesta, descripcion, categoria, re, pregunta, tipo_pregunta, opc_a, opc_b, opc_c, opc_d, opc_e, foto, tipo_campo, puntaje_por_pregunta, cuenta, habilitado) VALUES  (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
...
<body>
	...
	<script>
		$(document).ready(function() {
			$.ajax({
				type: "POST",
				url: "getters/get_table_encuestas_new.php",
				data: null,
				beforeSend: function() {
