...
if (isset($_POST["import"])) {
    ...
    if ($_FILES["file"]["size"] > 0) {
		...
		$sqlTruncate = "TRUNCATE table repositorio_territorio;";
		...
		while (($column = fgetcsv($file, 10000, ";")) !== FALSE) {
            ...
            $territorio = "";
			if (isset($column[1])) {
				$territorio = mysqli_real_escape_string($conn, $column[1]);
			}			
            ...
            $sqlInsert = "INSERT INTO repositorio_territorio (id,territorio,activo,eliminar) VALUES  (?,?,?,?)";
            $paramType = "isss";
            $paramArray = array(
                $id,
				$territorio,
...
<body>
	...
	<script>
		$(document).ready(function() {
			$.ajax({
				type: "POST",
				url: "getters/get_table_territorio.php",
				data: null,
				beforeSend: function() {
