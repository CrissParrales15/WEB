...
if (isset($_POST["import"])) {
    ...
    if ($_FILES["file"]["size"] > 0) {
		...
		$sqlTruncate = "TRUNCATE table repositorio_productos_onpacks;";
		...
		while (($column = fgetcsv($file, 10000, ";")) !== FALSE) {
			...
			$plataforma = "";
			if (isset($column[20])) {
				$plataforma = mysqli_real_escape_string($conn, $column[20]);
			}
            
			$pvs = "";
			if (isset($column[21])) {
				$pvs = mysqli_real_escape_string($conn, $column[21]);
			}
            
            $sqlInsert = "INSERT INTO repositorio_productos_onpacks (id,sector,categoria,subcategoria,segmento,presentacion,variante1,variante2,contenido,sku,marca,elaborado,activar,dolar,fabricante,historico,pvp,cadenas,locales,foto,plataforma,pvs) VALUES  (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
            $paramType = "isssssssssssssssssssss";
            $paramArray = array(
                $id,
				$sector,
				$categoria,
...
<body>
	...
	<script>
		$(document).ready(function() {
			$.ajax({
				type: "POST",
				url: "getters/get_table_productos_onpacks.php",
				data: null,
				beforeSend: function() {
