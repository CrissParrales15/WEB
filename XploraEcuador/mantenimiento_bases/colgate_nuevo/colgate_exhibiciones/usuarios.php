...
if (isset($_POST["import"])) {
    ...
    if ($_FILES["file"]["size"] > 0) {
		...
		$sqlTruncate = "TRUNCATE table repositorio_usuario;";
		...
		while (($column = fgetcsv($file, 10000, ";")) !== FALSE) {
			...
			$mercaderista = "";
			if (isset($column[2])) {
				$mercaderista = mysqli_real_escape_string($conn, $column[2]);
			}
			...
			$device_id = "";
			if (isset($column[4])) {
				$device_id = mysqli_real_escape_string($conn, $column[4]);
			}

			$color = "";
			if (isset($column[5])) {
				$color = mysqli_real_escape_string($conn, $column[5]);
			}
            ...
            $sqlInsert = "INSERT INTO repositorio_usuario (id,user,mercaderista,pass,device_id,color,status) VALUES (?,?,?,?,?,?,?)";
            $paramType = "sssssss";
            $paramArray = array(
				$id,
                $user,
...
<body>
    ...
    <div class="outer-scontainer">
		...
		<?php
            $sqlSelect = "SELECT * FROM repositorio_usuario";
