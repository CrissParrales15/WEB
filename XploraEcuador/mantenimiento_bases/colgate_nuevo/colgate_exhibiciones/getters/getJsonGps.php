<?php
	...
	$query_supervisor = "SELECT
						rs.id_supervisor AS id_supervisor, 
						rs.supervisor AS nombre_supervisor 
						FROM rutero_pdv r 
						INNER JOIN repositorio_locales_dtt2 rl ON r.id_pdv=rl.id 
						INNER JOIN repositorio_supervisor rs ON rl.supervisor=rs.id_supervisor 
						INNER JOIN repositorio_usuario ru ON r.id_usuario=ru.id 
						WHERE (r.fecha_visita BETWEEN ? AND ?) 
						AND ru.mercaderista NOT LIKE '%IMPULSO%' 
						AND ru.mercaderista NOT LIKE '%EMILY VILLAMAR%' 
						AND ru.mercaderista NOT LIKE '%PRUEBA%' 
						AND rs.rol='SUPERVISOR' 
						AND r.id_estado=? 
						AND r.status=1 
						AND r.habilitado=1 
	...
	if ($sql_supervisor = $mysqli->prepare($query_supervisor)) {
		$sql_supervisor->bind_param('ssi', $fechaInicio, $fechaFin, $estado);
		...
		if ($sql_supervisor->num_rows > 0) {
			...
			while($sql_supervisor->fetch()) {
				...
				$query = "SELECT 
						...
						WHERE r.id_supervisor=? AND (r.fecha_visita BETWEEN ? AND ?) 
						AND ru.mercaderista NOT LIKE '%IMPULSO%'
						AND ru.mercaderista NOT LIKE '%EMILY VILLAMAR%'
						AND ru.mercaderista NOT LIKE '%PRUEBA%'
						AND r.id_estado=? 
				...
				if ($sql = $mysqli->prepare($query)) {
					$sql->bind_param('issi', $id_supervisor, $fechaInicio, $fechaFin, $estado);
					$sql->execute();
					$sql->store_result();
					if ($sql->num_rows > 0) {
						$sql->bind_result(
