<?php
	...
	$query = "SELECT id, codigo, mercaderista, nombre_pdv, cadena, supervisor, fecha_visita, hora_llegada_min, hora_llegada, hora_llegada_max, hora_inicio_visita, hora_salida_min, hora_salida, hora_salida_max, hora_fin_visita, observacion_entrada, observacion_salida, leido FROM lvi_notificaciones_visitas_calificadas WHERE fecha_visita BETWEEN ? AND ?";
	if ($sql = $mysqli->prepare($query)) {
		$sql->bind_param('ss', $fechaInicio, $fechaFin);
		$sql->execute();
        $sql->store_result();
        if ($sql->num_rows > 0) {
