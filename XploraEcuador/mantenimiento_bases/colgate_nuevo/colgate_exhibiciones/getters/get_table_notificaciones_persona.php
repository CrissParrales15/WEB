<?php
	...
	$query = "SELECT cp_code,supervisor,exhibition_tool,nherramienta,realizadas,eliminadas,pendientes FROM lvi_exhibiciones_colgate WHERE supervisor = ?";
	if ($sql = $mysqli->prepare($query)) {
		$sql->bind_param('sss', $fechaInicio, $fechaFin, $supervisor);
		$sql->execute();
        $sql->store_result();
        if ($sql->num_rows > 0) {
