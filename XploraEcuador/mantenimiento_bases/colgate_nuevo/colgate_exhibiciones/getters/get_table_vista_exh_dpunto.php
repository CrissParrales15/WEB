<?php
	...
	$query = "SELECT cp_code,local,exhibition_tool,nherramienta,realizadas,eliminadas,pendientes FROM lvi_exhibiciones_colgate WHERE /*supervisor = ? AND*/ cp_code = ?";
	if ($sql = $mysqli->prepare($query)) {
		$sql->bind_param('s', $cp_code);
		$sql->execute();
        $sql->store_result();
        if ($sql->num_rows > 0) {
