...
if (isset($_POST["import"])) {
    ...
    if ($_FILES["file"]["size"] > 0) {
		...
		$sqlTruncate = "TRUNCATE table repositorio_tipo_inventario;";
		...
		while (($column = fgetcsv($file, 10000, ";")) !== FALSE) {
            ...
            $causales = "";
			if (isset($column[1])) {
				$causales = mysqli_real_escape_string($conn, $column[1]);
			}

            $sqlInsert = "INSERT INTO repositorio_tipo_inventario (id,causales) VALUES  (?,?)";
            $paramType = "is";
            $paramArray = array(
                $id,
				$causales
...
<body>
	...
	<script>
		$(document).ready(function() {
			$.ajax({
				type: "POST",
				url: "getters/get_table_tipo_inventario.php",
				data: null,
				beforeSend: function() {
