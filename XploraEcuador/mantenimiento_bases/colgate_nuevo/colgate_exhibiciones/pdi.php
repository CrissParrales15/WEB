...
if (isset($_POST["import"])) {
    ...
    if ($_FILES["file"]["size"] > 0) {
		...
		$sqlTruncate = "TRUNCATE table repositorio_pdi;";
		...
		while (($column = fgetcsv($file, 10000, ";")) !== FALSE) {
			...
			$marca = "";
			if (isset($column[4])) {
				$marca = mysqli_real_escape_string($conn, $column[4]);
			}

			$objetivo = "";
			if (isset($column[5])) {
				$objetivo = mysqli_real_escape_string($conn, $column[5]);
			}

			$plataforma = "";
			if (isset($column[6])) {
				$plataforma = mysqli_real_escape_string($conn, $column[6]);
			}
			
            $sqlInsert = "INSERT INTO repositorio_pdi(id, canal, categoria, subcategoria, marca, objetivo, plataforma) VALUES (?,?,?,?,?,?,?)";
            $paramType = "sssssss";
            $paramArray = array(
				$id,
                $canal,
...
<body>
    ...
    <div class="outer-scontainer">
		...
		<?php
            $sqlSelect = "SELECT * FROM repositorio_pdi";
