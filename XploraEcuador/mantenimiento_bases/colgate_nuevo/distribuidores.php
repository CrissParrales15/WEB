...
if (isset($_POST["import"])) {
    ...
    if ($_FILES["file"]["size"] > 0) {
		...
		$sqlTruncate = "TRUNCATE table repositorio_distribuidores;";
		...
		while (($column = fgetcsv($file, 10000, ";")) !== FALSE) {
            ...
            $distribuidor = "";
			if (isset($column[1])) {
				$distribuidor = mysqli_real_escape_string($conn, $column[1]);
			}

						
            $sqlInsert = "INSERT INTO repositorio_distribuidores (id,distribuidor) VALUES  (?,?)";
            $paramType = "is";
            $paramArray = array(
                $id,
				$distribuidor
...
<body>
	...
	<div id="content" style="width:100%;">
		<nav class="navbar navbar-expand-lg navbar-light bg-light">
			<div class="container-fluid">
				<h3>Base Distribuidor (NUEVO PDV)</h3>
			</div>
		</nav>
	...
	<script>
		$(document).ready(function() {
			$.ajax({
				type: "POST",
				url: "getters/get_table_distribuidores.php",
				data: null,
				beforeSend: function() {
