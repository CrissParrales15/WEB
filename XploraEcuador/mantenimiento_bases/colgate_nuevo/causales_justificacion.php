...
if (isset($_POST["import"])) {
    ...
    if ($_FILES["file"]["size"] > 0) {
		...
		$sqlTruncate = "TRUNCATE table repositorio_justificacion;";
		...
		while (($column = fgetcsv($file, 10000, ";")) !== FALSE) {
			...
			$tipo_usuario = "";
			if (isset($column[1])) {
				$tipo_usuario = mysqli_real_escape_string($conn, $column[1]);
			}
            $justificacion = "";
			if (isset($column[2])) {
				$justificacion = mysqli_real_escape_string($conn, $column[2]);
			}

            $sqlInsert = "INSERT INTO repositorio_justificacion (id,tipo_usuario,justificacion) VALUES  (?,?,?)";
            $paramType = "iss";
            $paramArray = array(
                $id,
				$tipo_usuario,
...
<body>
	...
	<div id="content" style="width:100%;">
		<nav class="navbar navbar-expand-lg navbar-light bg-light">
			<div class="container-fluid">
				<h3>Base Causales Justificación PDV</h3>
			</div>
		</nav>
	...
	<script>
		$(document).ready(function() {
			$.ajax({
				type: "POST",
				url: "getters/get_table_causales_justificacion.php",
				data: null,
				beforeSend: function() {
