...
if (isset($_POST["import"])) {
    ...
    if ($_FILES["file"]["size"] > 0) {
		...
		$sqlTruncate = "TRUNCATE table repositorio_exhibiciones;";
		...
		while (($column = fgetcsv($file, 10000, ";")) !== FALSE) {
            ...
            $re = "";
			if (isset($column[1])) {
				$re = mysqli_real_escape_string($conn, $column[1]);
			}

            $tipo_herramienta = "";
			if (isset($column[2])) {
				$tipo_herramienta = mysqli_real_escape_string($conn, $column[2]);
			}

			$tipo = "";
			if (isset($column[3])) {
				$tipo = mysqli_real_escape_string($conn, $column[3]);
			}

			$fabricante = "";
			if (isset($column[4])) {
				$fabricante = mysqli_real_escape_string($conn, $column[4]);
			}

			$categoria = "";
			if (isset($column[5])) {
				$categoria = mysqli_real_escape_string($conn, $column[5]);
			}

			$subcategoria = "";
			if (isset($column[6])) {
				$subcategoria = mysqli_real_escape_string($conn, $column[6]);
			}
			
			$activo = "";
			if (isset($column[7])) {
				$activo = mysqli_real_escape_string($conn, $column[7]);
			}
			
			$eliminado = "";
			if (isset($column[8])) {
				$eliminado = mysqli_real_escape_string($conn, $column[8]);
			}
			
			
            $sqlInsert = "INSERT INTO repositorio_exhibiciones (id,re,tipo_herramienta,tipo,fabricante,categoria,subcategoria,activo,eliminado) VALUES  (?,?,?,?,?,?,?,?,?)";
            $paramType = "issssssss";
            $paramArray = array(
                $id,
				$re,
...
<body>
	...
	<script>
		$(document).ready(function() {
			$.ajax({
				type: "POST",
				url: "getters/get_table_exhibiciones.php",
				data: null,
				beforeSend: function() {
