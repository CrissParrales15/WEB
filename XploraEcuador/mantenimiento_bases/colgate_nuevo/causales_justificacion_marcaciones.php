...
if (isset($_POST["import"])) {
    ...
    if ($_FILES["file"]["size"] > 0) {
		...
		$sqlTruncate = "TRUNCATE table repositorio_causales_efectividad;";
		...
		while (($column = fgetcsv($file, 10000, ";")) !== FALSE) {
            ...
            $causal = "";
			if (isset($column[1])) {
				$causal = mysqli_real_escape_string($conn, $column[1]);
			}
            
			$modificar_horas = "";
			if (isset($column[2])) {
				$modificar_horas = mysqli_real_escape_string($conn, $column[2]);
			}

            $sqlInsert = "INSERT INTO repositorio_causales_efectividad (id,causal, modificar_horas) VALUES  (?,?,?)";
            $paramType = "iss";
            $paramArray = array(
                $id,
				$causal,
...
<body>
	...
	<div id="content" style="width:100%;">
		<nav class="navbar navbar-expand-lg navbar-light bg-light">
			<div class="container-fluid">
				<h3>Causales Justificación Marcaciones</h3>
			</div>
		</nav>
	...
	<script>
		$(document).ready(function() {
			$.ajax({
				type: "POST",
				url: "getters/get_table_causales_justificacion_marcaciones.php",
				data: null,
				beforeSend: function() {
