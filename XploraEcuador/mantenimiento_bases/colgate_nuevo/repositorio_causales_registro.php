...
if (isset($_POST["import"])) {
    ...
    if ($_FILES["file"]["size"] > 0) {
		...
		$sqlTruncate = "TRUNCATE table repositorio_causales_registro;";
		...
		while (($column = fgetcsv($file, 10000, ";")) !== FALSE) {
			...
			$causales = "";
			if (isset($column[3])) {
				$causales = mysqli_real_escape_string($conn, $column[3]);
			}

            $sqlInsert = "INSERT INTO repositorio_causales_registro (id, rol, re, causales) VALUES (?,?,?,?)";
            $paramType = "ssss";
            $paramArray = array(
				$id,
				$rol,
...
<body>
	...
	<script>
		$(document).ready(function() {
			$.ajax({
				type: "POST",
				url: "getters/get_table_causales_registro.php",
				data: null,
				beforeSend: function() {
