...
if (isset($_POST["import"])) {
    ...
    if ($_FILES["file"]["size"] > 0) {
		...
		$sqlTruncate = "TRUNCATE table repositorio_tracking;";
		...
		while (($column = fgetcsv($file, 10000, ";")) !== FALSE) {
            ...
            $customer = "";
			if (isset($column[1])) {
				$customer = mysqli_real_escape_string($conn, $column[1]);
			}

			$mecanica = "";
			if (isset($column[2])) {
				$fabricante = mysqli_real_escape_string($conn, $column[2]);
			}
			...
			$descripcion = "";
			if (isset($column[4])) {
				$descripcion = mysqli_real_escape_string($conn, $column[4]);
			}

			$precio_promocion = "";
			if (isset($column[5])) {
				$precio_promocion = mysqli_real_escape_string($conn, $column[5]);
			}

			$vigencia = "";
			if (isset($column[6])) {
				$vigencia = mysqli_real_escape_string($conn, $column[6]);
			}

			$material_pop = "";
			if (isset($column[7])) {
				$material_pop = mysqli_real_escape_string($conn, $column[7]);
			}

			$activo = "";
			if (isset($column[8])) {
				$activo = mysqli_real_escape_string($conn, $column[8]);
			}

			$eliminar = "";
			if (isset($column[9])) {
				$eliminar = mysqli_real_escape_string($conn, $column[9]);
			}
			
			
            $sqlInsert = "INSERT INTO repositorio_tracking (id,customer,mecanica,categoria,descripcion,precio_promocion,vigencia,material_pop,activo,eliminar) VALUES  (?,?,?,?,?,?,?,?,?,?)";
            $paramType = "isssssssss";
            $paramArray = array(
                $id,
				$customer,
				$mecanica,
...
<body>
	...
	<script>
		$(document).ready(function() {
			$.ajax({
				type: "POST",
				url: "getters/get_table_tracking.php",
				data: null,
				beforeSend: function() {
