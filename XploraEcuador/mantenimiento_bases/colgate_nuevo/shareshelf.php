...
if (isset($_POST["import"])) {
    ...
    if ($_FILES["file"]["size"] > 0) {
		...
		$sqlTruncate = "TRUNCATE table repositorio_share_shelf;";
		...
		while (($column = fgetcsv($file, 10000, ";")) !== FALSE) {
			...
			$activo = "";
			if (isset($column[5])) {
				$activo = mysqli_real_escape_string($conn, $column[5]);
			}
			
			$eliminar = "";
			if (isset($column[6])) {
				$eliminar = mysqli_real_escape_string($conn, $column[6]);
			}
			
			
            $sqlInsert = "INSERT INTO repositorio_share_shelf (id,re,fabricante,categoria,subcategoria,activo,eliminar) VALUES  (?,?,?,?,?,?,?)";
            $paramType = "issssss";
            $paramArray = array(
                $id,
				$re,
...
<body>
	...
	<script>
		$(document).ready(function() {
			$.ajax({
				type: "POST",
				url: "getters/get_table_shareshelf.php",
				data: null,
				beforeSend: function() {
