...
if (isset($_POST["import"])) {
    ...
    if ($_FILES["file"]["size"] > 0) {
		...
		$sqlTruncate = "TRUNCATE table repositorio_tracking_comentario;";
		...
		while (($column = fgetcsv($file, 10000, ";")) !== FALSE) {
			...
			$comentario = "";
			if (isset($column[2])) {
				$comentario = mysqli_real_escape_string($conn, $column[2]);
			}

			$activo = "";
			if (isset($column[3])) {
				$activo = mysqli_real_escape_string($conn, $column[3]);
			}

			$eliminar = "";
			if (isset($column[4])) {
				$eliminar = mysqli_real_escape_string($conn, $column[4]);
			}
			
            $sqlInsert = "INSERT INTO repositorio_tracking_comentario (id,status,comentario,activo,eliminar) VALUES  (?,?,?,?,?)";
            $paramType = "issss";
            $paramArray = array(
                $id,
				$status,
...
<body>
	...
	<div id="content" style="width:100%;">
		<nav class="navbar navbar-expand-lg navbar-light bg-light">
			<div class="container-fluid">
				<h3>Base Tracking Comentarios</h3>
			</div>
		</nav>
	...
	<script>
		$(document).ready(function() {
			$.ajax({
				type: "POST",
				url: "getters/get_table_tracking_comentarios.php",
				data: null,
				beforeSend: function() {
