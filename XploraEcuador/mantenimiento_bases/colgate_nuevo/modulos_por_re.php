...
if (isset($_POST["import"])) {
    ...
    if ($_FILES["file"]["size"] > 0) {
		...
		$sqlTruncate = "TRUNCATE table repositorio_modulos_por_re;";
		...
		while (($column = fgetcsv($file, 10000, ";")) !== FALSE) {
			...
			$rol = "";
			if (isset($column[1])) {
				$rol = mysqli_real_escape_string($conn, $column[1]);
			}

			$re = "";
			if (isset($column[2])) {
				$re = mysqli_real_escape_string($conn, $column[2]);
			}

			$modulo = "";
			if (isset($column[3])) {
				$modulo = mysqli_real_escape_string($conn, $column[3]);
			}

			$nombre_modulo = "";
			if (isset($column[4])) {
				$nombre_modulo = mysqli_real_escape_string($conn, $column[4]);
			}
			
            $sqlInsert = "INSERT INTO repositorio_modulos_por_re (id, rol, re, modulo, nombre_modulo) VALUES (?,?,?,?,?)";
            $paramType = "sssss";
            $paramArray = array(
				$id,
				$rol,
...
<body>
	...
	<script>
		$(document).ready(function() {
			$.ajax({
				type: "POST",
				url: "getters/get_table_modulos_por_re.php",
				data: null,
				beforeSend: function() {
