...
if (isset($_POST["import"])) {
    ...
    if ($_FILES["file"]["size"] > 0) {
		...
		$sqlTruncate = "TRUNCATE table repositorio_mantenimiento_cp;";
		...
		while (($column = fgetcsv($file, 10000, ";")) !== FALSE) {
            ...
            $fabricante = "";
			if (isset($column[1])) {
				$fabricante = mysqli_real_escape_string($conn, $column[1]);
			}

			$tipo_novedad = "";
			if (isset($column[2])) {
				$tipo_novedad = mysqli_real_escape_string($conn, $column[2]);
			}

			
            $sqlInsert = "INSERT INTO repositorio_mantenimiento_cp (id,fabricante,tipo_novedad) VALUES  (?,?,?)";
            $paramType = "iss";
            $paramArray = array(
                $id,
				$fabricante,
...
<body>
	...
	<script>
		$(document).ready(function() {
			$.ajax({
				type: "POST",
				url: "getters/get_table_tiponovedad.php",
				data: null,
				beforeSend: function() {
