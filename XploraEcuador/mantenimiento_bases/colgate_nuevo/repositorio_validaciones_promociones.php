...
if (isset($_POST["import"])) {
    ...
    if ($_FILES["file"]["size"] > 0) {
		...
		$sqlTruncate = "TRUNCATE table repositorio_validaciones_promociones;";
		...
		while (($column = fgetcsv($file, 10000, ";")) !== FALSE) {
            ...
            $cadena = "";
			if (isset($column[1])) {
				$cadena = mysqli_real_escape_string($conn, $column[1]);
			}

            $actividad = "";
			if (isset($column[2])) {
				$actividad = mysqli_real_escape_string($conn, $column[2]);
			}
            ...
            $cod_sku = "";
			if (isset($column[4])) {
				$cod_sku = mysqli_real_escape_string($conn, $column[4]);
			}

            $producto = "";
			if (isset($column[5])) {
				$producto = mysqli_real_escape_string($conn, $column[5]);
			}

            $descuento = "";
			if (isset($column[6])) {
				$descuento = mysqli_real_escape_string($conn, $column[6]);
			}

			
            $sqlInsert = "INSERT INTO repositorio_validaciones_promociones (id, cadena, actividad, subcategoria, cod_sku, producto, descuento) VALUES (?, ?, ?, ?, ?, ?, ?)";
            $paramType = "issssss";
            $paramArray = array(
                $id,
				$cadena,
...
<body>
	...
	<div id="content" style="width:100%;">
		<nav class="navbar navbar-expand-lg navbar-light bg-light">
			<div class="container-fluid">
				<h3>Base Validaciones Promociones</h3>
			</div>
		</nav>
	...
	<script>
		$(document).ready(function() {
			$.ajax({
				type: "POST",
				url: "getters/get_table_validaciones_promociones.php",
				data: null,
				beforeSend: function() {
