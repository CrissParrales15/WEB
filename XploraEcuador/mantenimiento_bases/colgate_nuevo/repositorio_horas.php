...
if (isset($_POST["import"])) {
    ...
    if ($_FILES["file"]["size"] > 0) {
		...
		$sqlTruncate = "TRUNCATE table repositorio_hora_sync;";
		...
		while (($column = fgetcsv($file, 10000, ";")) !== FALSE) {
            ...
            $hora = "";
			if (isset($column[1])) {
				$hora = mysqli_real_escape_string($conn, $column[1]);
			}

			
            $sqlInsert = "INSERT INTO repositorio_hora_sync (id,hora) VALUES  (?,?)";
            $paramType = "is";
            $paramArray = array(
                $id,
				$hora
...
<body>
	...
	<script>
		$(document).ready(function() {
			$.ajax({
				type: "POST",
				url: "getters/get_table_horas.php",
				data: null,
				beforeSend: function() {
