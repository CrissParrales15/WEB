...
if (isset($_POST["import"])) {
    ...
    if ($_FILES["file"]["size"] > 0) {
		...
		$sqlTruncate = "TRUNCATE table repositorio_encuestas;";
		...
		while (($column = fgetcsv($file, 10000, ";")) !== FALSE) {
			...
			$link = "";
			if (isset($column[2])) {
				$link = mysqli_real_escape_string($conn, $column[2]);
			}
			
            $sqlInsert = "INSERT INTO repositorio_encuestas (id,re,link) VALUES  (?,?,?)";
            $paramType = "iss";
            $paramArray = array(
                $id,
				$re,
...
<body>
	...
	<script>
		$(document).ready(function() {
			$.ajax({
				type: "POST",
				url: "getters/get_table_encuestas.php",
				data: null,
				beforeSend: function() {
