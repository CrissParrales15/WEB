...
if (isset($_POST["import"])) {
    ...
    if ($_FILES["file"]["size"] > 0) {
		...
		$sqlTruncate = "TRUNCATE table repositorio_zonaexhibicion;";
		...
		while (($column = fgetcsv($file, 10000, ";")) !== FALSE) {
            ...
            $zonaexh = "";
			if (isset($column[1])) {
				$zonaexh = mysqli_real_escape_string($conn, $column[1]);
			}
            ...
            $eliminar = "";
			if (isset($column[3])) {
				$eliminar = mysqli_real_escape_string($conn, $column[3]);
			}
		
			
            $sqlInsert = "INSERT INTO repositorio_zonaexhibicion (id,zonaexh,activo,eliminar) VALUES  (?,?,?,?)";
            $paramType = "isss";
            $paramArray = array(
                $id,
				$zonaexh,
...
<body>
	...
	<script>
		$(document).ready(function() {
			$.ajax({
				type: "POST",
				url: "getters/get_table_zonaexh.php",
				data: null,
				beforeSend: function() {
