...
if (isset($_POST["import"])) {
    ...
    if ($_FILES["file"]["size"] > 0) {
		...
		$sqlTruncate = "TRUNCATE table repositorio_retail;";
		...
		while (($column = fgetcsv($file, 10000, ";")) !== FALSE) {
			...
			$retail = "";
			if (isset($column[2])) {
				$retail = mysqli_real_escape_string($conn, $column[2]);
			}

			$reabrev = "";
			if (isset($column[3])) {
				$reabrev = mysqli_real_escape_string($conn, $column[3]);
			}

			$formatocp = "";
			if (isset($column[4])) {
				$formatocp = mysqli_real_escape_string($conn, $column[4]);
			}

			$formato = "";
			if (isset($column[5])) {
				$formato = mysqli_real_escape_string($conn, $column[5]);
			}

			$target = "";
			if (isset($column[6])) {
				$target = mysqli_real_escape_string($conn, $column[6]);
			}

			$distribuidor = "";
			if (isset($column[7])) {
				$distribuidor = mysqli_real_escape_string($conn, $column[7]);
			}
            ...
            $sqlInsert = "INSERT INTO repositorio_retail (id,canal,retail,reabrev,formatocp,formato,target,distribuidor,activo,eliminar) VALUES  (?,?,?,?,?,?,?,?,?,?)";
...
<body>
	...
	<script>
		$(document).ready(function() {
			$.ajax({
				type: "POST",
				url: "getters/get_table_retail.php",
				data: null,
				beforeSend: function() {
