...
if (isset($_POST["import"])) {
    ...
    if ($_FILES["file"]["size"] > 0) {
		...
		$sqlTruncate = "TRUNCATE table repositorio_encuesta_supervisores;";
		...
		while (($column = fgetcsv($file, 10000, ";")) !== FALSE) {
            ...
            $link = "";
			if (isset($column[1])) {
				$link = mysqli_real_escape_string($conn, $column[1]);
			}

            $activo = "";
			if (isset($column[2])) {
				$activo = mysqli_real_escape_string($conn, $column[2]);
			}

            $eliminado = "";
			if (isset($column[3])) {
				$eliminado = mysqli_real_escape_string($conn, $column[3]);
			}
			
			
            $sqlInsert = "INSERT INTO repositorio_encuesta_supervisores (id,link,activo,eliminado) VALUES  (?,?,?,?)";
            $paramType = "isss";
            $paramArray = array(
                $id,
				$link,
...
<body>
	...
	<div id="content" style="width:100%;">
		<nav class="navbar navbar-expand-lg navbar-light bg-light">
			<div class="container-fluid">
				<h3>Base Encuesta Supervisor</h3>
			</div>
		</nav>
	...
	<script>
		$(document).ready(function() {
			$.ajax({
				type: "POST",
				url: "getters/get_table_encuesta_supervisores.php",
				data: null,
				beforeSend: function() {
