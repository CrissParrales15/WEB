...
if (isset($_POST["import"])) {
    ...
    if ($_FILES["file"]["size"] > 0) {
		...
		$sqlTruncate = "TRUNCATE table repositorio_usuarios;";
		...
		while (($column = fgetcsv($file, 10000, ";")) !== FALSE) {
			...
			$id_rol = "";
			if (isset($column[1])) {
				$id_rol = mysqli_real_escape_string($conn, $column[1]);
			}

			$usuario = "";
			if (isset($column[2])) {
				$usuario = mysqli_real_escape_string($conn, $column[2]);
			}
			...
			$nombre = "";
			if (isset($column[4])) {
				$nombre = mysqli_real_escape_string($conn, $column[4]);
			}

			$apellido = "";
			if (isset($column[5])) {
				$apellido = mysqli_real_escape_string($conn, $column[5]);
			}

			$mercaderista = "";
			if (isset($column[6])) {
				$mercaderista = mysqli_real_escape_string($conn, $column[6]);
			}

			$cedula = "";
			if (isset($column[7])) {
				$cedula = mysqli_real_escape_string($conn, $column[7]);
			}
			...
			$device_id = "";
			if (isset($column[9])) {
				$device_id = mysqli_real_escape_string($conn, $column[9]);
			}
			
			$color = "";
			if (isset($column[10])) {
				$color = mysqli_real_escape_string($conn, $column[10]);
			}

			$status = "";
			if (isset($column[11])) {
				$status = mysqli_real_escape_string($conn, $column[11]);
			}

			$activo = "";
			if (isset($column[12])) {
				$activo = mysqli_real_escape_string($conn, $column[12]);
			}
			
			$id_supervisor = "";
			if (isset($column[13])) {
				$id_supervisor = mysqli_real_escape_string($conn, $column[13]);
			}
			
            $sqlInsert = "INSERT INTO repositorio_usuarios(id,id_rol,usuario,password,nombre,apellido,mercaderista,cedula,telefono,device_id,color,status,activo,id_supervisor) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
            $paramType = "ssssssssssssii";
            $paramArray = array(
				$id,
				$id_rol,
                $usuario,
...
<body>
	...
	<script>
		$(document).ready(function() {
			$.ajax({
				type: "POST",
				url: "getters/get_table_usuarios.php",
				data: null,
				beforeSend: function() {
