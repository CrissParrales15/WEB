$(document).ready(()=>{
   ...
   /* supervisor = [
    ...
    let data_pdv = document.getElementById('data_pdv');
    let data_visitados = document.getElementById('data_visitados')
    let data_proceso = document.getElementById('data_proceso')
    let data_pendientes = document.getElementById('data_pendientes')
    let data_efectividad = document.getElementById('data_efectividad')
    let data_supervisor = document.getElementById('data_supervisores')
    let data_gestores = document.getElementById('data_gestores')
    ...
    a_head_panel_seguimiento.append(' SEGUIMIENTO GPS');
    ...
    */
    ...
    function setResizeGoogleMaps() {
        ...
        $('#gmap').height(viewportHeight);
        GoogleMaps.updateSize();
    };

    function scheduleGoogleMapsResize(delay) {
        setTimeout(function () {
            if (GoogleMaps && typeof GoogleMaps.updateSize === 'function') {
                setResizeGoogleMaps();
            }
        }, delay || 0);
    }
    ...
    $(window).on('resize orientationchange', function () {
        scheduleGoogleMapsResize(120);
    });

    if (window.visualViewport) {
        window.visualViewport.addEventListener('resize', function () {
            scheduleGoogleMapsResize(80);
        });
    }
    ...
    function obtenerCategoriasReportadas(pdvData, usuario, callback) {
        ...
        console.log("Todas las propiedades:", Object.keys(pdvData));
        
        // Determinar cuál campo usar para código
        ...
        console.log("=== DATOS ENVIADOS A getKPIS.php ===");
        console.log("URL completa:", "/App/XploraEcuador/mantenimiento_bases/colgate_nuevo/getters/getKPIS.php?" + $.param(dataToSend));
        console.log("Parámetros:", dataToSend);
        
        $.ajax({
            url: "/App/XploraEcuador/mantenimiento_bases/colgate_nuevo/getters/getKPIS.php",
            method: "GET",
            data: dataToSend,
            dataType: 'json',
            success: function(data) {
        ...
        $.each(data, function (i, v) {
            ...
            $.each(v.gestor, function (ii, iv) {
                ...
                $.each(iv.pdv, function (iii, iiv) {
                    ...
                    //#region  MODAL KPIS - EN CASO DE NO TENER CATEGORIAS REPORTADAS
                    btn_kpis.addEventListener('click', () => {
                        ...
                        // Usar la función
                        obtenerCategoriasReportadas(pdvData,/* cuenta,*/ gestorData.nombre, function(error, data) {
                            if (error) {
                                $('#kpis-modal .modal-body').html(`
                            ...
                            if (data && data.estados && Array.isArray(data.estados)) {
                                ...
                                formato = 'nuevo_completo';
                                kpis_categorias = data.estados.map(estado => ({
                                    ...
                                    // caduc: estado.propenso ? 1 : 0,
                                    // qs: estado.quiebre ? 1 : 0,
                                }));
                            } else if (data && Array.isArray(data) && data.length > 0) {
                                // FORMATO ANTIGUO o NUEVO SIMPLE
                                // if (typeof data[0] === 'object' && 'nombre' in data[0]) {
                                    // FORMATO NUEVO SIMPLE: [{nombre: "YOGURT", exh: 1, ...}, ...]
                                    formato = 'nuevo_simple';
                                    kpis_categorias = data;
                                // } else {
                                //     // FORMATO ANTIGUO: ["YOGURT", "AVENA", ...]
                                //     formato = 'antiguo';
                                //     kpis_categorias = data.map(categoriaNombre => ({
                            ...
                            // console.log("Categorías procesadas:", kpis_categorias);

                            // Si no hay categorías reportadas
                            if (kpis_categorias.length === 0) {
                                const kpisContent = `
                                    <div class="kpis-modal-content">
                                        <!-- DIV PRINCIPAL CON DOS COLUMNAS -->
                                        <div class="kpis-main-layout">
                                            <!-- COLUMNA IZQUIERDA: Punto de venta + Datos del gestor -->
                                            <div class="kpis-left-column">
                                                ...
                                                <div class="gestor-section">
                                                    <div class="gestor-header">
                                                        <strong>DATOS GENERALES DEL GESTOR</strong>
                                                    </div>
                                            ...
                                            <!-- COLUMNA DERECHA: Fotos HORIZONTALES -->
                                            <div class="kpis-right-column">
                                                <div class="fotos-container">
                                                    ...
                                                    <div class="foto-item">
                                                        ...
                                                        <div class="foto-box">
                                                            ${fotos.ENTRADA ? 
                                                                `<img src="${fotos.ENTRADA}" class="foto-img">` : 
                                                                '<p class="foto-placeholder">No disponible</p>'
                                                            }
                                                        </div>
                                                    </div>
                                                    ...
                                                    <div class="foto-item">
                                                        ...
                                                        <div class="foto-box">
                                                            ${fotos.SALIDA ? 
                                                                `<img src="${fotos.SALIDA}" class="foto-img">` : 
                                                                '<p class="foto-placeholder">No disponible</p>'
                                                            }
                                                        </div>
                                                    </div>
                                            ...
                                            <div class="fotos-container">
                                                ...
                                                <div class="foto-item">
                                                    ...
                                                    <div class="foto-box">
                                                        ${fotos.ENTRADA ? 
                                                            `<img src="${fotos.ENTRADA}" class="foto-img">` : 
                                                            '<p class="foto-placeholder">Foto entrada</p>'
                                                        }
                                                    </div>
                                                </div>
                                                ...
                                                <div class="foto-item">
                                                    ...
                                                    <div class="foto-box">
                                                        ${fotos.SALIDA ? 
                                                            `<img src="${fotos.SALIDA}" class="foto-img">` : 
                                                            '<p class="foto-placeholder">Foto salida</p>'
                                                        }
                                                    </div>
                                                </div>
                                            </div>
                                    ...
                                    <div class="kpis-table-section">
                                        ...
                                        <div class="kpis-table-container">
                                            <table class="kpis-table">
                                                ...
                                                <tbody>
                                                    ${kpis_categorias.map(cat => `
                                                        <tr>
                                                            <td style="font-weight: bold;">${cat.nombre}</td>
                                                            <td style="text-align: center;">${cat.tipo_cat || ''}</td>
                                                            <td style="text-align: center;" class="kpi-indicator ${cat.prec ? 'kpi-yes' : 'kpi-no'}">
                                                                ${cat.prec ? '<span class="check-box">✓</span>' : ''}
                                                            </td>
                                                            <td style="text-align: center;" class="kpi-indicator ${cat.presn ? 'kpi-yes' : 'kpi-no'}">
                                                                ${cat.presn ? '<span class="check-box">✓</span>' : ''}
                                                            </td>
                                                            <td style="text-align: center;" class="kpi-indicator ${cat.sos ? 'kpi-yes' : 'kpi-no'}">
                                                                ${cat.sos ? '<span class="check-box">✓</span>' : ''}
                                                            </td>
                                                            <td style="text-align: center;" class="kpi-indicator ${cat.promos ? 'kpi-yes' : 'kpi-no'}">
                                                                ${cat.promos ? '<span class="check-box">✓</span>' : ''}
                                                            </td>
                                                            <td style="text-align: center;" class="kpi-indicator ${cat.exh ? 'kpi-yes' : 'kpi-no'}">
                                                                ${cat.exh ? '<span class="check-box">✓</span>' : ''}
                                                            </td>
                                                    ...
                                                    ${(() => {
                                                        ...
                                                        const totalCategorias = kpis_categorias.length;
                                                        
                                                        // Contar checks por columna
                                                        const precCount = kpis_categorias.filter(cat => cat.prec).length;
                                                        const presnCount = kpis_categorias.filter(cat => cat.presn).length;
                                                        const sosCount = kpis_categorias.filter(cat => cat.sos).length;
                                                        const promosCount = kpis_categorias.filter(cat => cat.promos).length;
                                                        const exhCount = kpis_categorias.filter(cat => cat.exh).length;

                                                        
                                                        // Calcular porcentajes
                                                        const precPorcentaje = totalCategorias > 0 ? Math.round((precCount / totalCategorias) * 100) : 0;
                                                        const presnPorcentaje = totalCategorias > 0 ? Math.round((presnCount / totalCategorias) * 100) : 0;
                                                        const sosPorcentaje = totalCategorias > 0 ? Math.round((sosCount / totalCategorias) * 100) : 0;
                                                        const promosPorcentaje = totalCategorias > 0 ? Math.round((promosCount / totalCategorias) * 100) : 0;
                                                        const exhPorcentaje = totalCategorias > 0 ? Math.round((exhCount / totalCategorias) * 100) : 0;
                                                        ...
                                                        return `
                                                            <tr style="background-color: #f8f9fa; font-weight: bold; border-top: 2px solid #ddd;">
                                                                <td style="text-align: left; font-weight: bold;">% DE CATEGORIAS REPORTADAS:</td>
